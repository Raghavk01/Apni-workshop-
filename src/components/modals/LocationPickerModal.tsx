...
export const LocationPickerModal: React.FC = () => {
  ...
  }, [isOpen, userLocation.lat, userLocation.lng, userLocation.address, userLocation.areaName]);

  // Clean up Leaflet on modal close or unmount
  useEffect(() => {
    if (!isOpen) {
  ...
  // Initialize interactive Leaflet map inside modal when opened
  useEffect(() => {
    if (!isOpen) return;
    ...
    const timer = setTimeout(() => {
      ...
      // Safely remove any existing map instance on container
      if (mapInstanceRef.current) {
        try {
  ...
  // Reverse geocode and optionally re-center pin
  const handleUpdateCoordinates = async (lat: number, lng: number, updateMap: boolean = false) => {
    setPinLat(lat);
    setPinLng(lng);
    setIsReverseGeocoding(true);

    if (updateMap && mapInstanceRef.current && markerRef.current) {
      try {
