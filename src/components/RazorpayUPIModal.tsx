  ...
  const handlePayNow = () => {
    if (selectedMethod === "cod") {
      ...
      setTimeout(() => {
        ...
        showToast(`Pay at Bay / COD Selected for ₹${totalPayable.toLocaleString()}!`);
        createBookingAndDispatch();
        setTimeout(() => {
          setPaymentSuccess(false);
          closeModal("razorpay");
      ...
      <div className="bg-[#ffffff] w-full max-w-md rounded-t-3xl sm:rounded-3xl overflow-hidden shadow-2xl border border-[#e4e3db] flex flex-col max-h-[92vh]">
        ...
        <div className="bg-[#0c2340] text-white p-4 flex items-center justify-between border-b border-[#1b3a60]">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-[#3395ff] flex items-center justify-center font-black text-white text-[16px]">
              ₹
            </div>
            <div>
        ...
        {paymentSuccess ? (
          <div className="p-8 text-center space-y-3">
            <div className="w-16 h-16 rounded-full bg-[#cbffc2] text-[#005312] flex items-center justify-center mx-auto shadow-inner">
              <span className="material-symbols-outlined text-[36px]">check_circle</span>
            </div>
            <h4 className="font-extrabold text-[18px] text-[#1b1c17]">Payment Verified & Successful!</h4>
            <div className="bg-[#f6f4ec] p-3 rounded-2xl border border-[#e4e3db] text-left space-y-1.5 font-mono text-[11px]">
              <div className="flex justify-between text-[#707a6c]">
                <span>Razorpay Payment ID:</span>
                <span className="text-[#1b1c17] font-bold">{paymentDetails?.paymentId || "pay_Q1aZ890123"}</span>
              </div>
              <div className="flex justify-between text-[#707a6c]">
              ...
              <div className="flex justify-between text-[#707a6c]">
                ...
                <span className="text-[#0d631b] font-bold">HMAC-SHA256 Authentic</span>
              </div>
            </div>
            <span className="inline-block bg-[#f0eee6] px-3 py-1 rounded-full text-[11px] font-bold text-[#0d631b]">
          ...
          <div className="p-4 space-y-4 overflow-y-auto">
            ...
            <div className="grid grid-cols-5 gap-1 bg-[#f0eee6] p-1 rounded-xl">
              <button
                ...
                className={`py-1.5 rounded-lg text-[10px] font-bold transition-all flex flex-col items-center justify-center ${
              ...
              <div className="space-y-3">
                ...
                <div className="p-3.5 bg-[#f6f4ec] rounded-2xl border border-[#e4e3db] flex flex-col items-center justify-center text-center space-y-2.5">
                  <div className="flex items-center justify-between w-full px-1">
                    <span className="text-[10px] font-bold text-[#707a6c] uppercase tracking-wider">
                  ...
                  <div className="flex items-center gap-2 w-full pt-1">
                    ...
                    >
                      <span className="material-symbols-outlined text-[14px] text-[#707a6c]">content_copy</span>
                ...
                <div className="space-y-1 pt-1">
                  ...
                  <p className="text-[10px] text-[#707a6c]">
                    Changing this updates the QR code instantly so scanning sends funds to that UPI account.
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="grid grid-cols-2 gap-2">
                {["HDFC Bank", "ICICI Bank", "State Bank of India", "Axis Bank"].map((bank) => (
                  <button
                    key={bank}
                    type="button"
                    className="p-3 rounded-xl border border-[#e4e3db] bg-white text-[12px] font-semibold text-left hover:border-[#3395ff]"
                  >
                    {bank}
                  </button>
                ))}
            ...
            {selectedMethod === "emi" && (
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 gap-2">
                  <button
                    type="button"
                    onClick={() => showToast("Bajaj Finserv No-Cost EMI Selected (₹" + Math.round(totalPayable / 3) + "/mo for 3 Months)")}
                    className="p-3 rounded-xl border border-[#3395ff] bg-[#f0f7ff] text-left flex items-center justify-between"
                  >
                    <div>
                  ...
                  <button
                    type="button"
                    onClick={() => showToast("ZestMoney Pay Later Selected")}
                    className="p-2.5 rounded-xl border border-[#e4e3db] bg-white text-left hover:border-[#3395ff] flex items-center justify-between"
                  >
                    <div>
                  ...
                  <button
                    type="button"
                    onClick={() => showToast("Amazon Pay Later Selected")}
                    className="p-2.5 rounded-xl border border-[#e4e3db] bg-white text-left hover:border-[#3395ff] flex items-center justify-between"
                  >
                    <div>
                      <div className="text-[12px] font-bold text-[#1b1c17]">Amazon Pay Later / FlexiPay</div>
                      <div className="text-[10px] text-[#707a6c]">Instant approval up to ₹60,000 credit line</div>
                    </div>
                    <span className="text-[11px] font-bold text-[#0d631b]">Approved</span>
                  </button>
                </div>
              </div>
            )}
            ...
            {selectedMethod === "cod" && (
              <div className="bg-[#fcfaf2] p-3.5 rounded-xl border border-[#e8e2c8] space-y-2 text-[#484224]">
                <div className="flex items-center gap-2 text-[#0c2340] font-bold text-[12px]">
                  <span className="material-symbols-outlined text-[20px] text-[#0d631b]">handshake</span>
            ...
            <div className="bg-[#f0f7ff] p-3 rounded-xl border border-[#cbe2ff] flex items-start gap-2.5 text-[11px] text-[#004880]">
              <span className="material-symbols-outlined text-[18px] text-[#0066b3] flex-shrink-0">
                shield_with_heart
              </span>
              <div>
                <strong className="font-bold block text-[#003366]">Apni Escrow Protection</strong>
                Your money is securely held until you inspect and approve the completed service at your doorstep.
              </div>
            </div>
            ...
            <button
              ...
              className="w-full h-12 rounded-xl bg-[#0c2340] hover:bg-[#13335c] text-white font-bold text-[13px] flex items-center justify-center gap-2 shadow-md active:scale-98 transition-all disabled:opacity-75"
            >
              {isProcessing ? (
                <>
                  <span className="w-4 h-4 rounded-full border-2 border-white border-t-transparent animate-spin"></span>
                  <span>Connecting to NPCI UPI Gateway...</span>
                </>
              ) : (
                <>
