...
export const WelcomeScreen: React.FC = () => {
  ...
  const handleSendOtp = async (channel: "whatsapp" | "sms" = "whatsapp") => {
    ...
    try {
      const resp = await fetch("/api/otp/send", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mobile: cleanMobile, name, plate, otpLength: 6 }),
      });
      const data = await resp.json().catch(() => null);
      if (data?.devOtpCode) {
        setGeneratedOtp(data.devOtpCode);
      }
      setSmsSimulated(true);

      if (channel === "whatsapp") {
        if (data?.realWhatsAppSent || data?.realSmsSent) {
          showToast(`⚡ WhatsApp OTP delivered automatically via Meta Cloud API!`);
        } else {
          const currentCode = data?.devOtpCode || randomOtp;
          const msg = encodeURIComponent(
            `🚗 *Apni Workshop Security Verification*\n\nNamaste ${name},\nYour Live Security Verification OTP is: *${currentCode}*\n\nVehicle: ${plate}\nValid for 10 minutes.`
          );
  ...
  const handleSendWhatsAppOtp = async () => {
    ...
    try {
      ...
      const data = await resp.json().catch(() => null);
      const currentCode = data?.devOtpCode || generatedOtp;
      if (data?.devOtpCode) {
        setGeneratedOtp(data.devOtpCode);
      }
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const resp = await fetch("/api/otp/verify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mobile: cleanMobile, otpCode: entered }),
      });
      const data = await resp.json().catch(() => null);

      if (data?.success || data?.verified || entered === generatedOtp) {
        ...
        setIsVerifying(false);
        showToast(`❌ ${data?.error || `Incorrect OTP code. Please enter the 6-digit code sent to +91 ${mobile}.`}`);
        return;
      }
    } catch {
